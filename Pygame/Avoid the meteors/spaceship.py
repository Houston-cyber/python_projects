...
class SpaceShip(pygame.sprite.Sprite):
	def __init__(self,path,x_pos,y_pos):
		...
		self.shield_surface = pygame.image.load('img/shield.png')
		self.health = 5

	def update(self): #movement of spaceship
		self.rect.center = pygame.mouse.get_pos()
		self.screen_constrain()
		self.display_health()

	def screen_constrain(self): #spaceship boundries of game window
		if self.rect.right >= 1280:
			self.rect.right = 1280
		if self.rect.left <= 0:
			self.rect.left = 0

	def display_health(self): #spaceship health display (top left)
...
class Meteor(pygame.sprite.Sprite):
	def __init__(self,path,x_pos,y_pos,x_speed,y_speed): #x speed (left and right) y speed (downwards)
		super().__init__()
		self.image = pygame.image.load(path)
		self.rect = self.image.get_rect(center = (x_pos,y_pos))
...
spaceship = SpaceShip('img/spaceship.png',640,500)
...
while True: # Game loop
	for event in pygame.event.get(): # Check for events / Player input
		...
		if event.type == pygame.MOUSEBUTTONDOWN:
			new_laser = Laser('img/Laser.png',event.pos,15)
		...
		if event.type == METEOR_EVENT:
			random_meteor_image = random.choice(('img/Meteor1.png','img/Meteor2.png','img/Meteor3.png'))
